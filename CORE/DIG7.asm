section .bss
	input resb 64 ;reserves 32 bytes for user-input
	
section .data
	intro db "Darknet Index / Guide [DIG]", 10 ;intro message
	intro_len equ $ - intro ;length of the intro
	
	version db "Version [ASM x86_64] 7.23.16", 10 ;versioning
	version_len equ $ - version ;length of the version

	options db "ALL OPTIONS: SITES [S], HISTORY [H], DOCUMENATION [D], RESOURCES [R], EXIT [E]", 10 ;options
	options_len equ $ - options ;length of options
...
;---OPTIONS PRINT---
		mov rax, 1 ;syswrite
		mov rdi, 1 ;stdout
		mov rsi, options ;pointer to the options print
		mov rdx, options_len ;pointer to the lenth of the options
		syscall
